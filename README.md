...
组封存把同样的裁决扩展到多批次：**锁行、缺口裁决与封存写是同一个 PostgreSQL 事务**——先按
**ID 升序**对全组成员行取 `FOR UPDATE`（与单批次封存、分片提交共享同一套行锁），再从**锁后快照**
计算各成员缺口，齐备则在同一事务里直接 `UPDATE` 全组并收集 `RETURNING` 的 `sealed_at`，缺片则
整体回滚。锁不会在裁决与写入之间释放，因此 `[A,B]` 与 `[B,A]` 的反序组请求、单批次封存与最后一片
分片三方并发时只会串行等待、不会死锁：后拿锁的事务在 READ COMMITTED 下重新读到先提交者的终态，
返回的成功快照与数据库**逐成员一致**，不可能出现"200 报告成员仍 OPEN、数据库却已 SEALED"的
自相矛盾响应。组事务自身要么全组共同生效、要么一个不动；某个成员已被**先串行化**的并发单批次封存
盖章时，组封存按幂等成员对待（保留其原 `sealedAt`），其余 OPEN 成员仍在同一事务里共同封存，
已 `SEALED` 成员的 `sealedAt` 永不被覆盖。
